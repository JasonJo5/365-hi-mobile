...
function goToBirthdateStep() {
    ...
    if (rawInput.replace(/[^0-9]/g, '').length < 9) {
        showError('phone-error', t('account_err_phone'));
        return;
    }
...
function lookupCustomer() {
    ...
    if (!birthdate) {
        showError('birthdate-error', t('account_err_birthdate'));
        return;
    }
    ...
    btn.disabled = true;
    btn.textContent = t('account_checking');
    ...
    fetch('/.netlify/functions/lookup-customer', {
        ...
        .catch(function (error) {
            ...
            showError('birthdate-error', t('account_err_generic'));
        })
        .finally(function () {
            btn.disabled = false;
            btn.textContent = t('account_view_account');
        });
}
...
function renderDashboard(data, history) {
    document.getElementById('dash-name').textContent = data.name || t('account_default_name');
    ...
    badge.textContent = isActive ? t('account_status_active') : t('account_status_expired');
    ...
    if (!history || history.length === 0) {
        ...
        history.forEach(function (entry) {
            ...
            item.innerHTML =
                ...
                '<div class="flex-1">' +
                '<p class="font-bold text-on-surface text-sm">' + escapeHtml(entry.planName || t('account_generic_plan')) + '</p>' +
